  ...
  template:
    metadata:
      labels:
        app: backend
    spec:
      # --- [QUAN TRỌNG] CHÌA KHÓA ĐỂ TẢI ẢNH TỪ AWS ---
      # (Phải đặt ngang hàng với containers, KHÔNG được đặt bên trong)
      imagePullSecrets:
        - name: regcred
      ...
      containers:
        - name: backend
          ...
          image: ${ecr_url}/dolciluxe-backend:latest
          ports:
            - containerPort: 8080
          ...
          env:
            ...
            # --- DATABASE ATLAS (Giữ nguyên nếu không đổi) ---
            - name: MONGO_URI
            ...
            - name: VNP_RETURN_URL
              # 👇 [SỬA TỰ ĐỘNG] Dùng biến server_ip để Terraform điền IP mới vào
              value: "http://${server_ip}/payment-return"
            ...
            # --- CẤU HÌNH KHÁC ---
            - name: CLIENT_URL
              # 👇 [SỬA TỰ ĐỘNG] Dùng biến server_ip
